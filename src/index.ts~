...
import { ICard, TBasketList } from './types';
...
events.on(
	/^order\..*:change$/,
	(data: { field: keyof TBasketList; value: string }) => {
...
events.on('orderFormErrors:change', (error: Partial<TBasketList>) => {
...
events.on('contactsFormErrors:change', (error: Partial<TBasketList>) => {
