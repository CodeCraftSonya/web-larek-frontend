...
#### Класс WebLarekAPI
Отвечает за взаимодействие с API интернет-магазина. Он предоставляет методы для получения списка товаров, информации о конкретном товаре и оформления заказа.

- cdn: string – базовый URL для загрузки изображений товаров.
- baseUrl: string – базовый URL API.
- options?: RequestInit – необязательные настройки запросов.
- 
Методы
- getProductList(): Promise<ICard[]>
Получает список всех товаров.
Отправляет GET-запрос на /product.
Возвращает массив объектов ICard, дополняя их полем image с корректным URL изображения.
- getProduct(id: string): Promise<ICard>
Получает информацию о конкретном товаре по его id.
Отправляет GET-запрос на /product/{id}.
Возвращает объект ICard, добавляя корректный URL изображения.
...
Возвращает ISuccessOrder, содержащий информацию о сформированном заказе.
